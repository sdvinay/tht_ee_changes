...
class Weblog {
    ...
    function entries()
    {
        ...
        {        
			...
			else
			{
				...
				{
					if (preg_match("#^C(\d+)#", $IN->QSTR, $match) OR preg_match("#^".$this->reserved_cat_segment."/#", $IN->QSTR, $match))
					{
			...
			foreach ($query->result as $row)
			{
				...
				if ($reldata = @unserialize($row['rel_data']))
				{
					...
					if ($row['rel_type'] == 'blog')
					{
						...
						$this->categories = $reldata['categories'];
						$this->parse_weblog_entries();
    ...
    function fetch_custom_weblog_fields()
    {
        ...
        if (USER_BLOG === FALSE)
        {
            $query = $DB->query("SELECT DISTINCT field_group FROM exp_weblogs WHERE is_user_blog = 'n'");
        ...
        {
            $group_id = $DB->escape_str(UB_FIELD_GRP);
        }     
        
        
        $sql = "SELECT field_id, field_type, field_name FROM exp_weblog_fields";
    ...
    function fetch_categories()
    {
        ...
        $sql = "SELECT c.cat_name, c.cat_id, c.cat_image, c.parent_id,
						p.cat_id, p.entry_id
        ...
        foreach ($categories as $val)
        {
            ...
            {    
                if ($val == $row['entry_id'])
                {
                    $this->temp_array[$row['cat_id']] = array($row['cat_id'], $row['parent_id'], $row['cat_name'], $row['cat_image']);
    ...
    function build_sql_query($qstring = '')
    {
		...
		$sql_b = ($TMPL->fetch_param('category') || $cat_id != '' || $order == 'random') ? "DISTINCT(t.entry_id) " : "t.entry_id ";
        ...
        // BEGIN_THT: Added by Vinay on 6/13/04 to support 'emailname' parameter
        // Updated by Vinay on 1/16/06 due to match changes to SQL query
        if ($TMPL->fetch_param('emailname'))
        {
			//$sql .= "LEFT JOIN exp_member_data ON m.member_id = exp_member_data.member_id ";
        }
        // END_THT: End of Vinay's addition
        
        // BEGIN_THT: Added by Vinay on 3/8/05 to support 'columnname' parameter
        if ($TMPL->fetch_param('columnname'))
        {
		//	$sql .= "LEFT JOIN exp_weblog_data ON t.entry_id = exp_weblog_data.entry_id ";
        }
        // END_THT: End of Vinay's addition
...
// END_THT: DST fixes
            ...
            else
            {
                ...
                {   
					...
					$query = $DB->query("SELECT t.year, t.month ".$sql);
                ...
                {   
					...
					$query = $DB->query("SELECT t.year, t.month, t.day ".$sql);
		...
		else
		{
			...
			switch ($order)
			{
				...
				case 'edit_date' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.edit_date";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.edit_date";
								}
					break;
				case 'expiration_date' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.expiration_date";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.expiration_date";
								}
					break;
				case 'title' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.title";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.title";
								}
					break;
				case 'url_title' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.url_title";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.url_title";
								}
					break;
				case 'view_count' : 
								...
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.{$vc} {$sort}, t.entry_date {$sort}";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.{$vc} {$sort}, t.entry_date {$sort}";
								}
				...
				case 'comment_total' : 
								...
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.comment_total {$sort}, t.entry_date {$sort}";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.comment_total {$sort}, t.entry_date {$sort}";
								}
				...
				case 'most_recent_comment' : 
								...
								if ($sticky == 'off')
								{
									$end .= "ORDER BY t.recent_comment_date {$sort}, t.entry_date {$sort}";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, t.recent_comment_date {$sort}, t.entry_date {$sort}";
								}
				...
				case 'username' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY m.username";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, m.username";
								}
					break;
				case 'screen_name' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY m.screen_name";
								}
								else
								{
									$end .= "ORDER BY t.sticky desc, m.screen_name";
								}
					break;
				case 'custom_field' : 
								if ($sticky == 'off')
								{
									$end .= "ORDER BY  wd.field_id_".$corder;
								}
								else
								{
									$end .= "ORDER BY t.sticky desc,  wd.field_id_".$corder;
								}
					break;
				case 'random' : 
								$end .= "ORDER BY rand()";  
								$sort = FALSE;
					break;
				default       : $end .= "ORDER BY t.entry_date";
					break;
			}                    
		}
    ...
    function parse_weblog_entries()
    {
        ...
        {         
                ...
                {   
                    ...
                    {                    
						foreach ($cat_chunk as $catkey => $catval)
						{
							...
							{
								if (in_array($v['0'], $not_these))
								{
									continue;
								}
								elseif(sizeof($these) > 0 && ! in_array($v['0'], $these))
								{
									continue;
								}
            ...
            {    
				...
				// asside so that when the related stuff is parsed out we'll have it.
